...
-(void)testUserResources{
    __block BOOL finished = NO;
    NSString * remoteIDString  = [NSString stringWithFormat:@"%@", [IPKUser currentUser].remoteID];
    ...
    [[IPKHTTPClient sharedClient] getFavoritePagesForUserWithId:remoteIDString success:^(AFJSONRequestOperation *operation, id responseObject){
        ...
        for (NSDictionary * teamDict in [responseObject objectForKey:@"team"]) {
            ...
            STAssertFalse([[teamDict objectForKey:@"user_id"] isEqualToNumber:[IPKUser currentUser].remoteID], @"following pages should not have been created by the user requesting the pages %@", teamDict);
        }
        finished = YES;
    } failure:^(AFJSONRequestOperation *operation, NSError *error){
        STAssertTrue(NO, [error debugDescription]);        
...
-(void)testPublicUserAccess{
    __block BOOL finished = NO;
    NSString * userID = [NSString stringWithFormat:@"%@", [IPKUser currentUser].remoteID];
...
-(void)testScoops{
    ...
    __block BOOL finished = NO;
    NSString * userIDString = [NSString stringWithFormat:@"%@",[IPKUser currentUser].remoteID];
